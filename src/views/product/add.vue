...
<template>
  <el-dialog v-model="dialogVisible" :before-close="resetFormConfirm" :title="'新增商品'" width="1024px">
    <el-form
      :model="formData"
      ref="vForm"
      :rules="rules"
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleImageSuccess = (response) => {
      // 第一张图片直接放
      if (!state.formData.picture) {
        state.formData.picture = response.data
      } else {
        // 非第一张图片则用逗号拼接
        state.formData.picture += "," + response.data
      }
